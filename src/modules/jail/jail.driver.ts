...
export class NsJail extends EventEmitter {
  ...
  add(k: string, v: any = undefined, isShort = false) {
    if (isShort) {
  ...
  async spawn(config?: SpawnOptionsWithoutStdio): Promise<string> {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.nsjailProcess = spawn(NsJail.jail_path, this.toString().split(' '), { stdio: 'pipe' })
      ...
      this.nsjailProcess.addListener('exit', (code, signal) => {
        if (this.onNsjailExit) {
          this.onNsjailExit(code ?? 114514, signal ?? '')
        }
  ...
  getCommand() {
    return `${NsJail.jail_path} ${this.toString()}`
  }
